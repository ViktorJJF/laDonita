...
import api from '@/services/api/dishes';
import {
  buildSuccess,
  handleError,
...
const module = {
  ...
  mutations: {
    ...
    create(state, data) {
      state.dishes.unshift(data);
    },
    update(state, { id, data }) {
      const indexToUpdate = state.dishes.findIndex(member => member.id === id);
      state.dishes.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      const indexToDelete = state.dishes.findIndex(member => member.id === id);
      state.dishes.splice(indexToDelete, 1);
    },
  },
  getters: {},
};
