...
const module = {
  ...
  mutations: {
    ...
    update(state, { id, data }) {
      const indexToUpdate = state.orders.findIndex(member => member.id === id);
      state.orders.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      const indexToDelete = state.orders.findIndex(member => member.id === id);
