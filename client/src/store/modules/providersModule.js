...
import api from '@/services/api/providers';
import {
  buildSuccess,
  handleError,
...
const module = {
  ...
  mutations: {
    ...
    create(state, data) {
      state.providers.unshift(data);
    },
    update(state, { id, data }) {
      const indexToUpdate = state.providers.findIndex(
        member => member.id === id,
      );
      state.providers.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      const indexToDelete = state.providers.findIndex(
        member => member.id === id,
      );
      state.providers.splice(indexToDelete, 1);
    },
  },
  getters: {},
};
