...
import api from '@/services/api/supplies';
import {
  buildSuccess,
  handleError,
...
const module = {
  ...
  mutations: {
    ...
    create(state, data) {
      state.supplies.unshift(data);
    },
    update(state, { id, data }) {
      const indexToUpdate = state.supplies.findIndex(
        member => member.id === id,
      );
      state.supplies.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      const indexToDelete = state.supplies.findIndex(
        member => member.id === id,
      );
      state.supplies.splice(indexToDelete, 1);
    },
  },
  getters: {},
};
